...
function setTheme(theme){
 ...
 const b=$('themeToggle'); if(b)b.textContent=theme==='dark'?'☀ Light':'🌙 Dark';
}
setTheme(localStorage.getItem('placementTheme')||'dark');
document.addEventListener('DOMContentLoaded',()=>{$('themeToggle').onclick=()=>setTheme(document.documentElement.getAttribute('data-theme')==='dark'?'light':'dark');});
...
function card(p){
 ...
 return `<article class="job priority-${pv}"><div><div class="company">${esc(c.name||'Employer')}</div><h3>${esc(p.title)}</h3><div class="pills">${tags.join('')}</div><div class="sub">${esc(p.location||'UK')}${p.placement_length_months?` · ${p.placement_length_months} months`:''}${p.salary_text?` · ${esc(p.salary_text)}`:''}</div><p class="summary">${esc(summary.slice(0,220))}${summary.length>220?'…':''}</p>${a?`<div class="app-stage">My application: ${esc(a.status)}${a.next_action?` · Next: ${esc(a.next_action)}`:''}</div>`:''}</div><div class="side"><div class="priority-badge ${pc}">${priorityName(p)}</div><div class="stars ${pc}">${priorityStars(p)}</div><div class="priority-reason">${esc(p.priority_reason||c.priority_reason||'')}</div><div class="actions"><button class="detail-btn" onclick="openJob('${p.id}')">Job description</button><button class="track-btn" onclick="trackPlacement('${p.id}')">${a?'Update application':'Track application'}</button></div></div></article>`;
}
...
async function loadApplications(){state.applications.clear();if(!state.user){render();return;}const {data,error}=await client.from('applications').select('*').eq('user_id',state.user.id);if(!error)(data||[]).forEach(a=>state.applications.set(a.placement_id,a));render();}

window.openJob=id=>{
 const p=state.placements.find(x=>x.id===id);if(!p)return;const c=company(p),pc=priorityClass(p);
 $('jobDetail').innerHTML=`<div class="eyebrow">${esc(p.category||'PLACEMENT')}</div><h2>${esc(p.title)}</h2><p class="muted"><strong>${esc(c.name||'Employer')}</strong> · ${esc(p.location||'UK')}</p><div class="pills"><span class="pill ${statusClass(p)}">${esc(p.status)}</span>${p.is_rolling?'<span class="pill rolling">ROLLING — APPLY ASAP</span>':''}${p.deadline?`<span class="pill">Deadline ${esc(fmtDate(p.deadline))}</span>`:''}</div><div class="detail-grid"><div class="detail-box priority-detail ${pc}"><span>Priority</span><strong class="stars ${pc}">${priorityStars(p)} · ${priorityName(p)}</strong></div><div class="detail-box"><span>Duration</span><strong>${p.placement_length_months?`${p.placement_length_months} months`:'Check listing'}</strong></div><div class="detail-box"><span>Salary</span><strong>${esc(p.salary_text||'Not stated')}</strong></div><div class="detail-box"><span>Last verified</span><strong>${esc(fmtDate(p.last_verified_at)||'Not recorded')}</strong></div></div>${p.priority_reason?`<div class="detail-section"><h3>Why this is a priority</h3><p>${esc(p.priority_reason)}</p></div>`:''}${p.description?`<div class="detail-section"><h3>About the placement</h3><p>${esc(p.description)}</p></div>`:''}${p.responsibilities?`<div class="detail-section"><h3>Responsibilities</h3><p>${esc(p.responsibilities)}</p></div>`:''}${p.requirements||p.eligibility_text?`<div class="detail-section"><h3>Requirements / eligibility</h3><p>${esc(p.requirements||p.eligibility_text)}</p></div>`:''}${p.desirable_skills?`<div class="detail-section"><h3>Useful skills</h3><p>${esc(p.desirable_skills)}</p></div>`:''}${c.expected_open_note?`<div class="detail-section"><h3>Application timing</h3><p>${esc(c.expected_open_note)}</p></div>`:''}<a class="official" href="${esc(p.url)}" target="_blank" rel="noopener">Open official application ↗</a>`;
 $('jobModal').classList.remove('hidden');
};

window.trackPlacement=async id=>{if(!state.user){$('authModal').classList.remove('hidden');return;}const p=state.placements.find(x=>x.id===id),a=appFor(p);$('appPlacementId').value=id;$('appTitle').textContent=`${p.title} — ${company(p).name}`;$('appStatus').value=a?.status||'Interested';$('appliedAt').value=a?.applied_at?a.applied_at.slice(0,10):'';$('nextAction').value=a?.next_action||'';$('nextActionAt').value=a?.next_action_at?a.next_action_at.slice(0,16):'';$('appNotes').value=a?.notes||'';$('appMessage').textContent='';$('appModal').classList.remove('hidden');};

$('saveApplication').onclick=async()=>{if(!state.user)return;const placement_id=$('appPlacementId').value,status=$('appStatus').value;const payload={user_id:state.user.id,placement_id,status,applied_at:$('appliedAt').value?new Date($('appliedAt').value).toISOString():null,next_action:$('nextAction').value.trim()||null,next_action_at:$('nextActionAt').value?new Date($('nextActionAt').value).toISOString():null,notes:$('appNotes').value.trim()||null,updated_at:new Date().toISOString()};if(status==='Applied'&&!payload.applied_at)payload.applied_at=new Date().toISOString();const {error}=await client.from('applications').upsert(payload,{onConflict:'user_id,placement_id'});$('appMessage').textContent=error?error.message:'Saved.';if(!error){await loadApplications();setTimeout(()=>$('appModal').classList.add('hidden'),450);}};
